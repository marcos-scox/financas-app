...
export default function AssistantScreen() {
  ...
  return <ScreenContainer className="px-5 pt-5"><ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 32 }}>
    ...
    <Surface style={{ marginTop: 18, padding: 15, borderColor: colors.primary }}><Text style={{ color: colors.foreground, fontWeight: "900", fontSize: 16, marginBottom: 10 }}>Faça sua pergunta</Text><Field label="Sua pergunta" value={question} onChangeText={setQuestion} placeholder="Ex.: Como montar uma reserva?" multiline returnKeyType="send" onSubmitEditing={send} style={{ minHeight: 72, textAlignVertical: "top" }} /><PrimaryButton label={sending ? "ENVIANDO..." : "ENVIAR PERGUNTA"} icon="paperplane.fill" onPress={send} disabled={sending || !question.trim()} /></Surface>
    <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8, marginTop: 12, marginBottom: 15 }}><Pressable onPress={() => setQuestion("Como organizar minhas contas?")} style={{ paddingHorizontal: 12, paddingVertical: 9, borderRadius: 20, borderWidth: 1, borderColor: colors.border }}><Text style={{ color: colors.muted, fontSize: 11, fontWeight: "700" }}>Organizar contas</Text></Pressable><Pressable onPress={() => setQuestion("Como montar uma reserva?")} style={{ paddingHorizontal: 12, paddingVertical: 9, borderRadius: 20, borderWidth: 1, borderColor: colors.border }}><Text style={{ color: colors.muted, fontSize: 11, fontWeight: "700" }}>Montar reserva</Text></Pressable><Pressable onPress={() => setQuestion("O que devo saber antes de investir?")} style={{ paddingHorizontal: 12, paddingVertical: 9, borderRadius: 20, borderWidth: 1, borderColor: colors.border }}><Text style={{ color: colors.muted, fontSize: 11, fontWeight: "700" }}>Antes de investir</Text></Pressable></View>
    <View style={{ marginTop: 4 }}>{messages.map((message, index) => <View key={`${message.role}-${index}`} style={{ alignSelf: message.role === "user" ? "flex-end" : "flex-start", maxWidth: "88%", backgroundColor: message.role === "user" ? colors.primary : colors.surface, borderWidth: 1, borderColor: message.role === "user" ? colors.primary : colors.border, borderRadius: 18, borderBottomRightRadius: message.role === "user" ? 5 : 18, borderBottomLeftRadius: message.role === "assistant" ? 5 : 18, padding: 14, marginBottom: 10 }}><Text style={{ color: message.role === "user" ? "#FFFFFF" : colors.foreground, fontSize: 14, lineHeight: 20 }}>{message.text}</Text></View>)}</View>
