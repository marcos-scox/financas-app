...
export default function AccountsScreen() {
  const colors = useColors();
  const { bills, saveBill, deleteBill, toggleBillPaid } = useFinance();
  ...
  const remove = () => {
    if (!editing) return;
    Alert.alert("Excluir conta", `Excluir ${editing.title}?`, [{ text: "Cancelar", style: "cancel" }, { text: "Excluir", style: "destructive", onPress: async () => { await deleteBill(editing); setShowForm(false); resetForm(); } }]);
  };
