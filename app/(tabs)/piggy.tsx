...
export default function PiggyScreen() {
  ...
  const { piggies, savePiggy, deletePiggy } = useFinance();
  ...
  const remove = () => { if (!editing) return; Alert.alert("Excluir cofrinho", `Excluir ${editing.bank}?`, [{ text: "Cancelar", style: "cancel" }, { text: "Excluir", style: "destructive", onPress: async () => { await deletePiggy(editing); setShowForm(false); reset(); } }]); };
  ...
  return <ScreenContainer className="px-5 pt-5"><ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 32 }}>
    <AppHeader eyebrow="Reserva" title="Cofrinho" subtitle="Dê nome aos seus objetivos e saiba quanto já guardou." action={<Pressable onPress={() => { reset(); setShowForm(true); }} style={[styles.fab, { backgroundColor: colors.primary }]}><IconSymbol name="plus" size={22} color="#0A0A0E" /></Pressable>} />
    {!showForm ? <PrimaryButton label="Criar cofrinho" icon="plus" onPress={() => { reset(); setShowForm(true); }} /> : null}
    <Surface style={{ backgroundColor: colors.primary, borderColor: colors.primary, alignItems: "center", paddingVertical: 24 }}><IconSymbol name="lock.fill" size={27} color="#0A0A0E" /><Text style={{ color: "#0A0A0E", fontSize: 12, fontWeight: "900", marginTop: 10, letterSpacing: 1 }}>TOTAL GUARDADO</Text><Text style={{ color: "#0A0A0E", fontSize: 31, fontWeight: "900", marginTop: 5 }}>{formatCurrency(total)}</Text><Text style={{ color: "#30450F", fontSize: 12, marginTop: 5 }}>{piggies.length} {piggies.length === 1 ? "cofrinho" : "cofrinhos"}</Text></Surface>
    {showForm ? <Surface style={{ marginTop: 18 }}><SectionTitle title={editing ? "Editar cofrinho" : "Novo cofrinho"} action={<Pressable onPress={() => setShowForm(false)}><IconSymbol name="xmark" size={20} color={colors.muted} /></Pressable>} /><PrimaryButton label={editing ? "Salvar alterações" : "Criar cofrinho"} icon="checkmark.circle.fill" onPress={submit} />{editing ? <Pressable onPress={remove} style={{ paddingVertical: 13, alignItems: "center" }}><Text style={{ color: colors.error, fontWeight: "800" }}>Excluir cofrinho</Text></Pressable> : null}<Field label="Nome do banco" value={bank} onChangeText={setBank} placeholder="Ex.: Banco principal" autoFocus /><Field label="Quanto você tem?" value={amount} onChangeText={setAmount} placeholder="0,00" keyboardType="decimal-pad" /></Surface> : null}
